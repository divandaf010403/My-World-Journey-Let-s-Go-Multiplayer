...
//
// "Enable/Disable Headbob, Changed look rotations - should result in reduced camera jitters" || Custom Version
...
public class FirstPersonController : MonoBehaviour
{
    ...
    void Start()
    {
        // if(!view.IsMine)
        // {
        //     Destroy(GetComponentInChildren<>());
        // }
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            ...
            // Rotate the player object on the Y axis (left-right movement)
            transform.localEulerAngles = new Vector3(0, yaw, 0);

            // Rotate the camera object on the X axis (up-down movement)
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("By Divanda Firdaus", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        GUILayout.Label("Custom Version 2.1", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        EditorGUILayout.Space();
        ...
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Network", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();

        fpc.view = (PhotonView)EditorGUILayout.ObjectField(new GUIContent("PhotonView", "PhotonView attached to the controller."), fpc.view, typeof(PhotonView), true);
        GUI.enabled = true;
